menu-title = Главное меню:
menu-find = 🔎 Найти авто
menu-bookings = 📄 Мои брони
menu-help = ℹ️ Помощь
menu-language = 🌐 Язык / Language / Til
# === start & language ===
start-pick-language = Выберите язык / Select language / Tilni tanlang:
start-welcome = Добро пожаловать! Нажмите «{menu-find}», чтобы подобрать машину.
session-expired = Сессия устарела. Пожалуйста заново нажмите «Найти авто» и выберите даты.

# статус в списке "Мои брони"
status-pending = На проверке
status-confirmed = Подтверждена
status-issued = Оформлена
status-paid = Оплачена
status-canceled = Отменена
status-rejected = Отклонена
status-expired = Истекла

search-context-actions = Вы можете изменить класс или даты поиска:
menu-change-class = 🚗 Изменить класс авто
menu-change-dates = 📅 Изменить даты
menu-pay = 💳 Оплатить
pay-select-type = Выберите способ оплаты
pay-full = Полная оплата
pay-advance = Аванс
pay-select-provider = Выберите платёжную систему
pay-link = Перейдите по ссылке для оплаты: { $url }
back = ◀️ Назад


errors-missing-booking = Произошла ошибка в бронировании.

client-booking-confirmed =
    ✅ Ваша бронь подтверждена!
    🚗 Авто: { $title } • Цвет: { $car_color } • Гос. номер: { $car_plate_number }
    🆔 Номер заявки: #{ $id }
    📅 Период: { $date_from } – { $date_to }
    ℹ️ Партнёр может связаться с вами для уточнения деталей. Пожалуйста, будьте на связи.

client-booking-paid =
    💳 Оплата успешно прошла!
    🚗 Авто: { $title } • Цвет: { $car_color } • Гос. номер { $car_plate_number }
    🆔 Номер заявки: #{ $id }
    📅 Период: { $date_from } – { $date_to }
    👤 Партнёр: { $partner_name }
    ☎️ Телефон: { $partner_phone }
    📍  Адрес: { $partner_address }
    ✅ Ваша бронь полностью подтверждена. Желаем приятной поездки!

client-booking-rejected =
    ❌ К сожалению, ваша заявка была отклонена.
    🚗 Авто: { $title } • Цвет: { $car_color } • Гос. номер: { $car_plate_number }
    🆔 Номер заявки: #{ $id }
    📅 Период: { $date_from } – { $date_to }
    ℹ️ На эти даты автомобиль недоступен. Мы подобрали для вас несколько похожих вариантов ниже.

client-booking-expired =
    ⏳ Время ожидания подтверждения истекло.
    🚗 Авто: { $title } • Цвет: { $car_color } • Гос. номер { $car_plate_number }
    🆔 Номер заявки: #{ $id }
    📅 Период: { $date_from } – { $date_to }
    ℹ️ Бронь была автоматически отменена. Мы предложим вам похожие автомобили на эти даты.

client-booking-suggest-item =
    • { $title } — { $price_weekday } сум/сутки (будни), { $price_weekend } сум/сутки (выходные)

client-booking-suggest-list =
    🔁 Похожие доступные автомобили:
    { $cars }
    ℹ️ Если ни один вариант не подошёл, попробуйте изменить даты или класс автомобиля.

client-booking-suggest-empty =
    😔 На выбранные даты не удалось найти подходящие свободные автомобили.
    ℹ️ Попробуйте изменить даты или выбрать другой класс автомобиля.


# заголовок "мои брони"
my-head = Список Ваших броней:
my-no-items = У вас нет активных бронирований. Нажмите «{ $menu_find }», чтобы найти авто.
my-line = #{ $id } • { $title } • { $status }\n{ $from_ } → { $to }

my-error = Ошибка: { $error }

# показ после отправки брони партнёру
book-sent =
    Заявка на аренду машины «{ $title }»
    с { $start } до { $end } отправлена.
    Статус: На проверке.
    Вы получите уведомление при подтверждении/отклонении.

book-create-error = Не удалось создать заявку. Попробуйте позже.\n{ $error }

# превью перед подтверждением клиентом
book-preview-head = Вы выбрали «{ $title }», { $start }–{ $end }.
book-preview-sum  = Ориентировочно: ≈ { $sum } UZS за { $days } дн.
book-preview-ask  = Отправить запрос?
book-btn-confirm = ✅ Отправить
book-btn-cancel  = ❌ Отмена

book-cancelled = Отменено.

errors-missing-dates = Не хватает данных для бронирования. Повторите поиск.

# choose pay type (оставляем как было)
pay-choose = Выберите тип оплаты:\n
pay-gw-picked = Вы выбрали оплату через { $gw } ({ $mode }).\nНомер вашей заявки: #{ $bid }.
pay-instruction = Нажмите кнопку, чтобы перейти на страницу оплаты.
pay-go = 💳 Оплатить
pay-no-link = Не удалось получить ссылку на оплату.
pay-back = Назад

lang-set-ok =
    { $done ->
        [uz] Язык установлен: Oʻzbekcha. Теперь нажмите «{menu-find}».
        [en] Language set to English. Now tap “{menu-find}”.
       *[ru] Язык установлен: Русский. Теперь нажмите «{menu-find}».
    }

# === phone & registration ===
phone-send = 📱 Отправить телефон
phone-again = Отправьте телефон кнопкой ниже или введите в формате +998XXXXXXXXX.
reg-ask-first = Введите ваше имя:
reg-first-short = Имя слишком короткое. Повторите, пожалуйста.
reg-ask-last = Введите вашу фамилию:
reg-last-short = Фамилия слишком короткая. Повторите, пожалуйста.
reg-ok = Регистрация завершена ✅
reg-fail = Регистрация не удалась: { $error }

reg-ask-birth = Введите вашу дату рождение.\nВ формате 01.12.2025
reg-birth-invalid = Неверный формат даты. Введите в формате ДД.ММ.ГГГГ\nНапример 01.12.2025
reg-birth-too-young = Заявки принимаются только от клиентов старше 18 лет.
reg-ask-drive-exp = Укажите ваш стаж вождения (в годах, только цифры).
reg-drive-exp-invalid = Пожалуйста, введите стаж вождения только цифрами, без букв и других символов.

# === legal ===
legal-offer = 📄 Публичная оферта
legal-privacy = 🔒 Политика конфиденциальности
legal-agree = ✅ Согласен
legal-decline = ✖️ Отмена
legal-prompt = Для продолжения необходимо согласиться с Публичной офертой и Политикой конфиденциальности. Ознакомьтесь и нажмите «✅ Согласен».
legal-send-offer-fail = Не удалось отправить файл оферты.
legal-offer-missing = Файл оферты не найден.
legal-send-privacy-fail = Не удалось отправить файл политики.
legal-privacy-missing = Файл политики не найден.
legal-declined = Вы отказались от условий. Без согласия регистрация невозможна.

# === calendar ===
cal-today = Сегодня
cal-tomorrow = Завтра
cal-weekdays = Пн,Вт,Ср,Чт,Пт,Сб,Вс

# === search flow ===
search-date-from = 📅 Выберите дату начала аренды:
search-date-to = 📅 Дата начала: { $start }\nТеперь выберите дату окончания аренды:
search-warn-past = Нельзя выбирать прошлую дату
search-warn-end-gt-start = Окончание должно быть позже начала
search-period = 🗓 Период: { $start } → { $end }\n\nВыберите класс авто:
search-results-none = К сожалению, подходящих авто не найдено. Попробуйте другой класс или измените даты.
search-results-head = Найдено { $count } авто.{ $extra }
showing-first-10 = Показаны первые 10.
    }
search-classes-head = Выберите другой класс или измените даты:

class-eco = Эконом
class-comfort = Комфорт
class-business = Бизнес
class-premium = Премиум
class-suv = Внедорожник
class-minivan = Минивэн
back-to-dates = « Назад к датам

# === car card ===
card-top = Регион: { $region }\nНомер автомобиля: { $plate_number }\n🚗 { $title }{ $year_part } • Пробег:{ $mileage_part } км • Цвет: { $color }
card-line2 = ⚙️ { $class_part }{ $drive_part }{ $gearbox_part }
engine_volume_text = объём двигателя: { $engine_volume_l }
fuel_cons_text = Расход топлива: { $cons } л/100 км
card-price = 🗓 Будни: { $wd } сум/сутки\n📅 Выходные: { $we } сум/сутки
card-terms = 💳 Депозит: { $deposit }\n💳 Аванс: { $advance }\n✒️ Лимит пробега: { $limit } км/сутки\n🛡️ Страховка: { $ins }
card-options-title = 🎁 Опции:
card-option-child = • 👶 Детское кресло (+по тарифу)
card-option-delivery = • 📍 Доставка/возврат по адресу
card-option-driver = • 🚘 Авто с водителем
card-fallback = { $caption }
card-age = Возраст клиента: от { $age } лет
card-drive-exp = Опыт вождения: от { $years } лет
card-passport-required = Обязательно: паспорт или ID-карта
label-class = Класс: { $value }
label-drive = Привод: { $value }
label-gear = Трансмиссия: { $value }

drive-fwd = Передний
drive-rwd = Задний
drive-awd = Полный

gearbox_at = Автомат
gearbox_mt = Механика
gearbox_amt = Робот
gearbox_cvt = Вариатор

fuel-petrol = Бензин
fuel-diesel = Дизель
fuel-gas = Газ
fuel-hybrid = Гибрид
fuel-electric = Электро

deposit-none = Без депозита
advance-none = Без аванса
deposit-low = Низкий залог
deposit-high = Высокий залог

ins-included = включена
ins-excluded = не включена

btn-more = 📷 Фото салона
btn-terms = 📋 Условия
btn-reviews = 💬 Отзывы
btn-book = ✅ Забронировать

# === terms dialog & errors ===
terms-title = 📋 Условия аренды для «{ $title }»:
terms-deposit = • 💳 Депозит: { $deposit }
terms-advance = • 💳 Аванс: { $advance }
terms-limit = • ✒️ Лимит пробега: { $limit } км/сутки (свыше — по тарифу партнёра)
terms-ins = • 🛡️ Страховка: { $ins }
terms-driver = • 🚘 Авто с водителем: { $has ->
    [yes] да
   *[no]  нет
}
terms-delivery = • 📍 Доставка: { $has ->
    [yes] доступна
   *[no]  нет
}
terms-child = • 👶 Детское кресло: { $has ->
    [yes] есть
   *[no]  нет
}
terms-age = Минимальный возраст клиента: { $age } лет
terms-drive-exp = Минимальный стаж вождения: { $years } лет
terms-passport = Паспорт/ID карта: { $has ->
    [yes] да
   *[no]  нет
}
terms-no-more-photos = Дополнительных фото нет.
terms-car-not-found = Авто не найдено

reviews-soon = Раздел с отзывами скоро будет доступен.
errors-car-not-found = Авто не найдено

# === booking ===
reg-before-booking = Пожалуйста зарегистрируйтесь для отправки заявок на бронирования автомобиля.
book-confirm-q = Вы выбрали: { $title }\nПериод: { $start } → { $end }\nОриентировочно: ~{ $sum } UZS за { $days } д.\n\nПодтвердить отправку заявки партнёру?
book-ask-contact = Вы выбрали { $title }, { $start }–{ $end }\nОставьте контакты для связи:
book-btn-confirm = ✅ Подтвердить
book-btn-cancel = ✖️ Отмена
book-canceled = Заявка отменена.
book-create-fail = Не удалось создать заявку: { $error }
book-created = Заявка на аренду машины «{ $title }» с { $start } до { $end } отправлена партнёру. Статус: На проверке.\nВы получите уведомление при подтверждении/отклонении.
selfie-ask = Для отправки брони партнёру нужно короткое селфи 📸\nФото используется только для подтверждения личности.
selfie-invalid = Отправьте, пожалуйста, обычное фото (JPG/PNG), не стикер и не видео.
selfie-save-fail = Селфи не удалось сохранить: {error}

status-paid = ✅ Оплата по заявке #{id} («{$title}») получена. Спасибо!

# === payments ===
pay-mode-full = 💳 Полная сумма ({ $amount } UZS)
pay-mode-adv = 🔖 Аванс ({ $amount } UZS)

pay-choose-type = 💰 Выберите способ оплаты:
pay-choose-provider = 💳 Выберите платёжную систему:

pay-open-link = 🔗 Нажмите на ссылку ниже, чтобы перейти к оплате:\n\n👉 [💳 Оплатить]({ $pay_url})
pay-amount-zero = ⚠️ Невозможно создать платёж: сумма равна нулю.

pay-choose-full = Выберите платёжную систему для полной оплаты:
pay-choose-adv = Выберите платёжную систему для оплаты аванса:
pay-choose = Выберите тип оплаты:
pay-back = Вернитесь к предыдущему сообщению и выберите вариант оплаты.
pay-gw-picked = Вы выбрали оплату через { $gw } ({ $mode }).\n\nНомер вашей заявки: #{ $bid }.
pay-full-btn = 💳 Оплатить полную сумму ({ $total } UZS)
pay-adv-btn = 💸 Оплатить аванс ({ $adv } UZS)
pay-go = 💳 Оплатить
pay-instruction = Нажмите кнопку, чтобы перейти на страницу оплаты.
pay-no-link = Ссылка для оплаты недоступна. Попробуйте позже.
pay-create-error = Ошибка создании транзакции.

# === suggestions ===
suggest-none = Подходящих альтернатив не найдено. Попробуйте изменить даты или класс авто.
suggest-head = Возможно, вам подойдут эти варианты:
suggest-tail = Чтобы продолжить поиск, нажмите «{menu-find}».

# === notifications ===
notify-confirmed = ✅ Бронь #{ $id } по авто «{ $title }» на { $start }–{ $end } подтверждена партнёром.
notify-rejected = ❌ Бронь #{ $id } по авто «{ $title }» на { $start }–{ $end } отклонена партнёром.
notify-expired = ⏳ Бронь #{ $id } по авто «{ $title }» на { $start }–{ $end } истекла/отменена.

client-booking-paid =
    ✅ Оплата брони #{ $id } по авто «{ $title }»
    на { $date_from }–{ $date_to } успешно получена.
    Бронь активна, приятной поездки!
